...
const SMSList = () => {
    ...
    const fetchData = async () => {
        setData([
            {"id": faker.datatype.uuid(), "name": faker.name.findName(), "status": "Active"},
            {"id": faker.datatype.uuid(), "name": faker.name.findName(), "status": "Active"},
            {"id": faker.datatype.uuid(), "name": faker.name.findName(), "status": "Disabled"},
        ]);
    }
        ...
        <div>
            ...
            <div className="row d-flex align-items-center">
                <div className="col-md-12">
                    <h5 className="title">{t('SMS Gateways')}</h5>
                </div>
            </div>
            ...
            <div className="table-responsive">
                <table className="table align-middle table-hover mb-0">
                    ...
                    <tbody>
                        {(
                            data.map((host, index) => (
                                <SMSListRow key={index} host={host} handleSendModal={() => sendEmailModal(host)}/>
                            ))
                        )}
                    </tbody>
                </table>
