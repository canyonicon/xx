...
// modal
// import {Modal, Body, Footer} from '../../../portals/Modal';
...
const Customers = () => {
    ...
    const loadCustomers = (page, search = null, sort = null, statusFilter = null) => {
        const toastLoading = toast.loading(t('Please wait...'), {position: 'top-center'});
        let url = 'http://127.0.0.1:8000/api/customers?page='+page;
        ...
        .then((res) => {
            ...
            setSortables(res.data.sortables)
            setStatusFilter(res.data.statusFilter)
            setPage(page)
    ...
    const changePage = (newPage) => {
        // check if new page is not equal length of links then load new page
    ...
    const handleSearchKeyPress = (event) => {
        if (event.key === 'Enter') {
            loadCustomers(page, event.target.value)
        }
    };
    ...
    const handleSort = (sort) => {
        // console.log(sort);
        loadCustomers(page, null, sort)
    }

    // statusFilter
    const handleStatusFilter = (statusFilter) => {
        loadCustomers(page, null, null, statusFilter)
    }
    
    // parent checkbox
    const handleParent = (event) => {
        setIsParentChecked(event.target.checked);
        // check all checkboxes if parent checkbox is checked
        ...
        if (parent.checked) {
            ...
            children.forEach(child => {
                ...
                // store child.value to new array
                newDeleted.push(child.value);
                
            });
            
            // add newDeleted to deleted array
            ...
            children.forEach(child => {
                ...
                // remove child.value from deleted array
                const index = deleted.indexOf(child.value);
                if (index > -1) {
    ...
    const handleChild = (event) => {
        ...
        if (event.target.checked) {
            ...
            // add event.target.value to deleted array
            setDeleted([...deleted, event.target.value])
            
        }else{
            // remove event.target.value from deleted array
            const index = deleted.indexOf(event.target.value);
            if (index > -1) {
                deleted.splice(index, 1);
            }
            setDeleted(deleted);
    ...
    const deleteCustomers = () => {
        ...
        axios.post('http://127.0.0.1:8000/api/customers/delete', {ids: deleted})
        .then(response => {
            if(response.data.status){
                ...
                toast.error(t('Script error 1'));
            }
        })
        .catch(error => {
            toast.error(t('Script error 2'));
        ...
        <div>
            ...
            <div className="row d-flex align-items-center">
                <div className="col-md-6">
                    <h5 className="title">{t("Customers")}</h5>
                </div>
                ...
                <div className="row d-flex align-items-center">
                    <div className="col-md-3">
                        <div className="search">
                            <input type="search" onKeyUp={handleSearchKeyPress} className='ps-4 form-control form-control-sm search-bg' placeholder='Search' />
                        </div>
                    </div>
                    ...
                    <div className="col-md-9">
                        <div className="d-flex gap-1 align-items-center">
                            ...
                            {/* delete */}
                            <button className={isParentChecked ? 'bg-danger button ms-auto' : 'button ms-auto invisible'} type="button" onClick={deleteCustomers}>
                                <b className="bi bi-trash fs-6 text-white"></b>
                            </button>

                            <button className="button" type="button" onClick={loadCustomers}>
                                <b className="bi bi-bootstrap-reboot fs-6"></b>
                            </button>
                            ...
                            <div className="dropdown filter">
                                ...
                                <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                    <li><a className="dropdown-item cursor">{t('All')}</a></li>
                                    {
                                        Object.entries(statusFilter).map(([key, value]) => {
                                            return (
                                                <li key={key}><a className="dropdown-item cursor" onClick={() => handleStatusFilter(key)}>{value}</a></li>
                                            );
                                        })
                                    }
                                </ul>
                            ...
                            <div className="dropdown sort">
                                ...
                                <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                    {
                                         Object.entries(sortables).map(([key, value]) => {
                                            return (
                                                <div key={key}>
                                                    {/* Using both key and value */}
                                                    <li><a className="dropdown-item cursor" onClick={() => handleSort(key)}>{value} {t("ASC")}</a></li>
                                                    <li><a className="dropdown-item cursor" onClick={() => handleSort("-"+key)}>{value} {t("DESC")}</a></li>
                                                </div>
                                            );
                                        })
                                    }
                        ...
                        <tr>
                            <th scope="col">
                                <div className="form-check">
                                    <input className="form-check-input" name="" id="parent" onChange={handleParent} type="checkbox" value="checkedValue" aria-label="Text for screen reader" />
                                </div>
                            </th>
                            <th scope="col">{t('Customer ID')}</th>
                            <th scope="col">{t('Name')}</th>
                            <th scope="col">{t('ID/Passport')}</th>
                    ...
                    <tbody>
                        {
                            customers.map((customer) => {
                                return <CustomerRow key={customer.id} customer={customer} handleChildCheck={handleChild}/>
                            })
                        }
                    </tbody>
                </table>
            ...
            {/* check if links lenght is greater then 0 then lood Pagination */}
            {
                links.length > 0 && <Pagination links={links} handlePagination={changePage} />
            }

            {/* offcanvas */}
            <Offcanvas isOpen={isModal2Open} title='New Passenger' handleClose={closeModal2}>
                <AddCustomerModal />
            </Offcanvas>
